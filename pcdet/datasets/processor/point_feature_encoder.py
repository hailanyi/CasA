...
class PointFeatureEncoder(object):
    def __init__(self, config, point_cloud_range=None,num_frames=1):
        super().__init__()
    ...
    def forward(self, data_dict):
        ...
        if 'points_src' in data_dict:
            data_dict['points_src'], use_lead_xyz = getattr(self, self.point_encoding_config.encoding_type)(
                data_dict['points_src']
            )

        data_dict['use_lead_xyz'] = use_lead_xyz
        for i in range(1,self.num_frames):
            if 'points'+str(-i) in data_dict.keys():
                data_dict['points'+str(-i)], _ = getattr(self, self.point_encoding_config.encoding_type)(data_dict['points'+str(-i)])
